Shader "Moons/Liquid/Liquid Outline"
{
	Properties
	{
		[Header(Main colour)]
		...
		[Toggle]_Texturecolouroutline("Texture colour outline", Float) = 0
	...
	SubShader
	{
		...
		void outlineSurf( Input i, inout SurfaceOutput o )
		{
			...
			float4 appendResult12 = (float4(_DistortionspeedX , _DistortionspeedY , 0.0 , 0.0));
			...
			float4 texture294 = temp_output_33_0;
			o.Emission = lerp(( _Outlinebrightness * _Outlinecolour ),texture294,_Texturecolouroutline).rgb;
		}
		ENDCG
		...
		uniform float _Texturecolouroutline;
		...
		void vertexDataFunc( inout appdata_full v, out Input o )
		{
			...
			float3 outline55 = 0;
			v.vertex.xyz += ( vertex_offset240 + float4( outline55 , 0.0 ) ).xyz;
		}
