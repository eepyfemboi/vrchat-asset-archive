Shader "UBER - Specular Setup/ Core"
{
	Properties
	{
		...
		_SpecColor2("Specular2", Color) = (1,1,1) 
	...
	CGINCLUDE
		...
		// with FAKE_RETROREFLECTION defined we can invert the way diffuse scattering works - can be useful to get effect similar to street signs that strongly "glow" in the car headlights
		//#define FAKE_RETROREFLECTION
		...
		// self-shadows from ONE realtime light (choose the light and attach UBER_applyLightForDeferred.cs script to it)
		...
		// if defined we use this color channel to determine initial tangent Z coord inside POM volume
		// will take effect only with specific POM shaders that has "Depth per vertex" or "Extruded mesh" flags set on
		#define VERTEX_COLOR_CHANNEL_POMZ a		
	...
	SubShader
	{
		Tags { "RenderType"="Opaque" "Queue"="Geometry+1" "PerformanceChecks"="False" "REFR"="Off" "TWO_LAYERS"="Off"}
		LOD 700
		...
		Pass
		{
			...
			// keywords available in regular mode
			#pragma shader_feature ___ _PARALLAXMAP _PARALLAX_POM _PARALLAX_POM_ZWRITE _PARALLAX_POM_SHADOWS
			// keywords available in 2 layers mode
		...
		*/			
		...
		Pass {
			...
			// keywords available in regular mode
			#pragma shader_feature ___ _PARALLAX_POM_ZWRITE
			// keywords available in 2 layers mode
			//#pragma shader_feature ___ _PARALLAXMAP _PARALLAXMAP_2MAPS
		...
		Pass
		{
			...
			#pragma shader_feature ___ _PARALLAXMAP
			// we don't have it in 2-layers mode
			#pragma shader_feature ___ _DETAIL_SIMPLE _DETAIL_TEXTURED _DETAIL_TEXTURED_WITH_SPEC_GLOSS
		...
		// pierceable depth write pass
		Pass {
			Name "PIERCEABLE_DEPTH"
			Tags { "LightMode" = "Vertex" } // we don't use vertex shading actually, but this will prevent Unity from calling this pass when it's not tagged at all

			CGPROGRAM 
			#pragma vertex vertPierceable
			#pragma fragment fragPierceable 

			// w/o zwrite it also works OKish, so don't make it unnecessary expensive here
			//#pragma shader_feature ___ _PARALLAX_POM_ZWRITE
