...
Shader "Lux/Translucent/Lux Trans Bumped Specular" {
	...
	//	#define LUX_AO_OFF
		...
		void vert (inout appdata_full v, out Input o) {
			...
			#ifdef POINT
				float3 myLightCoord = mul(unity_WorldToLight, mul(unity_ObjectToWorld, v.vertex)).xyz;
			...
			#endif
			#ifdef SPOT
				float4 myLightCoord = mul(unity_WorldToLight, mul(unity_ObjectToWorld, v.vertex));
