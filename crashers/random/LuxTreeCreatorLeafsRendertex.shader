...
//#define LUX_LINEAR
//#define LUX_LLFIX_BILLBOARDS_ON
...
fixed4 frag (v2f i) : COLOR {
	...
	fixed3 albedo = col.rgb; // * i.color; / i.color will be applied to ambient
	...
	//	add diffuse IBL / always globally controlled
		...
		ambient = diff_ibl.rgb * ExposureIBL.x * i.color * 0.5; // as it gest multiplied by 2 later on
	#else
...
/// lightcolors are passed using the wrong colorspace
		...
		half nl = i.nl[j];
		// diffuse
		//light += lightColor * (translucencyColor + nl);
		half nh = i.nh[j];
		//spec += pow (nh, specular) * gloss * lightColor;
...
//c.rgb = pow (i.nh[1], specular) * 2; // just black
//c.rgb = specular * 0.125 * pow(i.nh[0], specular) * 2; // here i get something...
// but it does not fit the mesh tree’s spec, so it is probably not worth it
	c.a = 1;
	return c;
}
ENDCG
