...
Shader "Bender's Shaders/All in One stencil" {
    Properties {
        ...
        _CellShadingAmount ("Cell Shading Amount", Range(-3, 3)) = 0
...
////// Lighting:
                float attenuation = LIGHT_ATTENUATION(i);
                float3 finalColor = ((attenuation*_ShadingStrength)*_LightColor0.rgb*floor(max(0,dot(lightDirection,i.normalDir)) * _Bands) / (_Bands - 1));
