...
// Variable Version by Serenade
Shader "CubedParadox/Flat Lit Toon Distance Fade-In V2" {
    Properties {
        _MainTex ("MainTex", 2D) = "white" {}
        _Color ("Color", Color) = (1,1,1,1)
        _ColorMask ("ColorMask", 2D) = "black" {}
        _DitherAmount ("Dither Amount", Range(0, 1)) = 0.25
		//Addedcode
		_StartRange ("Visible Range", Range(0, 25)) = 1.0
		_TransitionRange ("Transition Range", Range(-1, 1)) = 0.5
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
				...
				//Alteredcode
                clip((_StartRange - (distance(i.posWorld.rgb,_WorldSpaceCameraPos)*lerp(1.0,node_8453,_DitherAmount))) - _TransitionRange);
////// Lighting:
